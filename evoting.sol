...
contract Election {
    ...
    */
    ...
    function addVoter(address _voter, string memory voter_id_number, address owner)
        public
        checkAdmin(owner)
...

    {
        ...
        for(uint i = 1; i<= voter_count; i++)
        {
            address x=voterID[i];
            require(keccak256(abi.encodePacked(voters[x].voterIDNumber))!=keccak256(abi.encodePacked(voter_id_number)),"ID already registered with an address");
            }
        voter_count++;
        voterID[voter_count] = _voter;
        voters[_voter].weight = 1;
        voters[_voter].isRegistered = true;
        voters[_voter].voterIDNumber= voter_id_number;
